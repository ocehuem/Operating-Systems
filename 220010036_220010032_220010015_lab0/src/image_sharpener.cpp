...
struct image_t* S3_sharpen(struct image_t *input_image, struct image_t *details_image) {
    ...
    for (int i = 0; i < input_image->height; ++i) {
        for (int j = 0; j < input_image->width; ++j) {
            for (int k = 0; k <= 2; ++k) {
                sharpened_image->image_pixels[i][j][k] = input_image->image_pixels[i][j][k] + 0.1 * details_image->image_pixels[i][j][k];
            }
        }
    }
...
int main(int argc, char **argv) {
    ...
    microseconds read_duration = duration_cast<microseconds>(t1_end - t1_start);
    ...
    microseconds s1_duration = duration_cast<microseconds>(t2_end - t2_start);

    // Apply S2 (find details)
    ...
    microseconds s2_duration = duration_cast<microseconds>(t3_end - t3_start);

    // Apply S3 (sharpen)
    high_resolution_clock::time_point t4_start = high_resolution_clock::now();
    struct image_t *sharpened_image = S3_sharpen(input_image, details_image);
    high_resolution_clock::time_point t4_end = high_resolution_clock::now();
    microseconds s3_duration = duration_cast<microseconds>(t4_end - t4_start);

    // Write the output image
    high_resolution_clock::time_point t5_start = high_resolution_clock::now();
    write_ppm_file(argv[2], sharpened_image);
    high_resolution_clock::time_point t5_end = high_resolution_clock::now();
    microseconds write_duration = duration_cast<microseconds>(t5_end - t5_start);
    ...
    double read_duration_sec = read_duration.count() / 1000000.0;
    double s1_duration_sec = s1_duration.count() / 1000000.0;
    double s2_duration_sec = s2_duration.count() / 1000000.0;
    double s3_duration_sec = s3_duration.count() / 1000000.0;
    double write_duration_sec = write_duration.count() / 1000000.0;
    double total_duration_sec = total_duration.count() / 1000000.0;

    // Print the timings in seconds
    cout << "file read: " << read_duration_sec << " seconds" << endl;
    cout << "S1 (smoothen): " << s1_duration_sec << " seconds" << endl;
    cout << "S2 (find details): " << s2_duration_sec << " seconds" << endl;
    cout << "S3 (sharpen): " << s3_duration_sec << " seconds" << endl;
    cout << "file write: " << write_duration_sec << " seconds" << endl;
